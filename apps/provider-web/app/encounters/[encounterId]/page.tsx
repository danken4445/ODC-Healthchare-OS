...
export default function EncounterRecordingPage() {
  ...
  async function issueEncounterPrescription(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await issuePrescription(createBrowserSupabaseClient(), {
      encounterId,
      medication: String(fields.get("medication") ?? "").trim(),
      dosage: String(fields.get("dosage") ?? "").trim(),
  ...
  async function issueEncounterCertificate(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await issueMedicalCertificate(createBrowserSupabaseClient(), {
      ...
      statement: String(fields.get("statement") ?? "").trim(),
    });
    setBusy(false);
    if (result.error)
        ...
        <div className="encounter-layout">
          ...
          <section className="encounter-recording" aria-labelledby="encounter-recording-heading">
            <div className="encounter-section-heading">
              <div><p className="eyebrow">Current encounter</p><h2 id="encounter-recording-heading">Clinical documentation</h2></div>
              <span>{currentSoapNote ? "Revision" : "New note"}</span>
            </div>
            <form className="encounter-note-form" onSubmit={saveNote}>
              <label htmlFor="encounter-soap-note">SOAP documentation</label>
              <textarea
                ...
                rows={18}
                maxLength={20000}
                defaultValue={currentSoapNote ? clinicalText(currentSoapNote.value) : ""}
                placeholder={"Subjective:\n\nObjective:\n\nAssessment:\n\nPlan:"}
                required
              />
              <div className="encounter-note-actions">
                <p>Saved notes are versioned in the patient record.</p>
                <Button disabled={busy || encounter.status !== "in_progress"} type="submit">{busy ? "Saving…" : "Save consultation note"}</Button>
              </div>
            </form>
                  ...
                  {canPrescribe && (
                    <section className="encounter-action-card">
                      ...
                      <form className="stack" onSubmit={issueEncounterPrescription}>
                        <Field label="Medication"><Input name="medication" maxLength={240} autoComplete="off" required /></Field>
                  ...
                  {canPrescribe && (
                    <section className="encounter-action-card">
                      ...
                      <form className="stack" onSubmit={issueEncounterCertificate}>
                        ...
                        <Field label="Statement"><textarea className="odyssey-input" name="statement" rows={5} maxLength={5000} required /></Field>
                  ...
                  {canOrderDiagnostics && (
                    <section className="encounter-action-card">
                      ...
                      <form className="stack" onSubmit={createEncounterRequest}>
                        ...
                        <Field label="Laboratory service"><select className="odyssey-input" name="laboratoryServiceId" defaultValue="" required><option disabled value="">Select a laboratory service</option>{laboratoryServices.filter((service) => service.active).map((service) => <option key={service.id} value={service.id}>{service.name} · PHP {service.labCost.toFixed(2)}</option>)}</select></Field>
                        <PriorityField />
                  ...
                  {canOrderDiagnostics && (
                    <section className="encounter-action-card">
                      ...
                      <form className="stack" onSubmit={createEncounterRequest}>
                        ...
                        <Field label="Specialist" hint="The affiliated clinic or hospital is shown with each specialist."><select className="odyssey-input" name="specialistRoleId" value={selectedSpecialistRoleId} onChange={(event) => setSelectedSpecialistRoleId(event.target.value)} required><option disabled value="">Select a specialist</option>{specialists.map((specialist) => <option key={specialist.practitionerRoleId} value={specialist.practitionerRoleId}>{specialist.displayName} · {specialist.organizationName}</option>)}</select></Field>
                        <PriorityField />
                        <Field label="Clinical note"><textarea className="odyssey-input" name="note" rows={3} maxLength={5000} /></Field>
                  ...
                  {canTagInventory && (
                    <section className="encounter-action-card">
                      ...
                      <form className="stack" onSubmit={tagEncounterItem}>
                        <Field label="Department"><select className="odyssey-input" name="departmentId" value={departmentSelection} onChange={(event) => setDepartmentSelection(event.target.value)} disabled={Boolean(departmentId)} required><option disabled value="">Select a department</option>{inventory?.departments.filter((department) => department.active).map((department) => <option key={department.id} value={department.id}>{department.name} ({department.code})</option>)}</select></Field>
                        <Field label="Item and available stock"><select className="odyssey-input" name="stockId" defaultValue="" required><option disabled value="">Select available stock</option>{inventory?.stock.filter((stock) => Number(stock.quantity) > 0 && (!departmentSelection || stock.department_id === departmentSelection)).map((stock) => { const item = inventory.items.find((candidate) => candidate.id === stock.item_id); const department = inventory.departments.find((candidate) => candidate.id === stock.department_id); return <option key={stock.id} value={stock.id}>{item?.name ?? "Item"} · {department?.name ?? "Department"} ({Number(stock.quantity).toLocaleString()} {item?.unit_of_measure ?? "units"})</option>; })}</select></Field>
                        <Field label="Quantity used"><Input name="quantity" type="number" min="0.001" step="0.001" defaultValue="1" required /></Field>
